...
import {
  createThreadSchema,
  updateThreadSchema,
} from "../utils/validator/ThreadValidator";
...
export default new (class ThreadServices {
  ...
  async find(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Succes Getting All Threads", threads });
    } catch (error) {
  ...
  async findOne(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Succes Getting a Thread", threads });
    } catch (error) {
      return res.status(500).json(error);
    }
  }
  ...
  async delete(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(200)
        .json({ messagae: "Succses Delete Thread", thread });
    } catch (error) {
      return res.status(500).json(error);
    }
  }
